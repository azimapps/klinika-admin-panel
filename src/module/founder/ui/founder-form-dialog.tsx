...
const FounderSchema = zod.object({
  ...
  linked_url: zod
    .string()
    .url({ message: "Noto'g'ri URL formati!" })
    .optional()
    .or(zod.literal('')),
...
export function FounderFormDialog({ open, onClose, currentRow }: Props) {
  ...
  const { mutateAsync: updateFounder, isPending: updatePending } = useUpdateFounder(
        ...
        <DialogContent>
          <Stack spacing={3} sx={{ mt: 2 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', mt: 3 }}>
              <Typography variant="subtitle2" sx={{ mb: 1, display: 'none' }}>
                {t('avatar')}
              </Typography>
              <Field.UploadAvatar
                name="avatar"
