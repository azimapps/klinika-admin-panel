...
const STATUS_COLOR_MAP: Record<ConsultationRequestStatusName, 'warning' | 'success' | 'info' | 'error' | 'default'> = {
    new: 'warning',
    reviewed: 'info',
    sent: 'warning',
    invoiced: 'warning',
...
export function ConsultationRequestDetailDialog({
    ...
    const canSendInvoice = statusName === 'new' || statusName === 'reviewed' || statusName === 'expired' || statusName === 'sent' || statusName === 'invoiced';
        ...
        <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
            ...
            <DialogContent>
                <Stack spacing={2.5} sx={{ pt: 1 }}>
                    ...
                    {/* Expired notice */}
                    {statusName === 'expired' && (
                        <Alert severity="warning">{t('expired_notice')}</Alert>
                    )}

                    {/* Accepted notice */}
                    {statusName === 'accepted' && (
                        <Alert severity="success">{t('accepted_notice')}</Alert>
                    )}
