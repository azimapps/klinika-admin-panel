...
export function ClinicFormDialog({ open, onClose, currentRow }: Props) {
  ...
  const { mutateAsync: updateClinic, isPending: updatePending } = useUpdateClinic(
  ...
  const handleSelectLocation = useCallback(
    (coords: [number, number], addresses: { uz: string; ru: string; en: string }) => {
            ...
            <Stack spacing={2}>
              ...
              >
                <YMaps>
                  ...
                  >
                    {values.lat && values.lon && <Placemark geometry={[values.lat, values.lon]} />}
                  </YMap>
                </YMaps>
                <Box
                  sx={{
